...
## Фаза 1 (качване на части)

| # | Дупка | Заобикаляне | Правилно решение |
|---|-------|-------------|------------------|
| G62 | черновата е файл под `uploads/<id>` и няма readdir, затова изоставена сесия не се чисти при триене на пространство | `DELETE /v1/fs/upload?id=` маха частта и meta-та. Крайният blob минава през обичайния store (и S3, ако е включен) | таблица `fs_uploads` и чистене заедно с workspace-а, когато схемата има място под 600 реда |
| G63 | multipart е само CRLF и `filename=""`. `filename*` и LF-only граници не се четат. Тялото на заявката пак се събира цяло, преди handler-а | част с `filename` се записва на `?path=`. Името от формата не става път | streaming parser в httpdbaga, ако една заявка трябва да е по-голяма от паметта |
...
| G59 | S3 клиентът е само path-style и чете отговора до EOF; няма virtual-hosted, chunked и multipart. rocksbaga и blob в boilaDB не са вързани | `SECP_BLOB=s3` плюс `S3_*`. Празен `SECP_BLOB` е локалният FS. Виж s3baga A2/A3 | отделен datasource, когато тези хранилища потрябват |
| G60 | span-ът се праща в края на заявката и start/end са почти равни; WebSocket и S3 не носят `traceparent` | header-ът на HTTP отговора е връзката. `SECP_OTEL_URL` праща OTLP/JSON с parent от входящия header | прокарване на контекста през blob слоя и през WS handshake |
| G61 | bulkhead-ът брои едновременни заявки в процеса, не опити за парола. Ако handler-ът вдигне ефект преди `leave`, слотът остава зает до рестарт | 429 при пълен таван. Пробите `/health` `/ready` `/metrics` не взимат слот. G46 за `/s/{token}/unlock` остава | брояч на опитите per линк; слот, който се връща и при ефект |
