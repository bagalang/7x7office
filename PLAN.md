...
- [x] ACL таблица по възли (2026-09-24): `tree_acl` (subject: user/role, path,
      `acl_level` 0..3 = none/read/write/share, `inherit`). Логиката „кой
      печели" е в `tree/acl_roles.baga` — ЧИСТ модул, тестван в
      `tests/acl_roles_test.baga` (наследяване по път, най-специфичният ред
      печели, „/a" не хваща „/ab", owner/admin не се самозаключва). DB
      достъпът е в `tree/acl_model.baga`; `tree_auth(ctx, db, need, path)`
      прилага редовете върху ролята за конкретния път. API: `GET/POST/DELETE
      /v1/fs/acl` (само owner/admin; личното пространство не се споделя;
      права към липсващ път → 404; `level` извън 0..3 → 400; ред за самия
      себе си → 400). Търсенето също уважава ACL (`act_search` филтрира
      резултатите, за да не изтичат имена на заключени файлове). Изтрит
      възел/поддърво маха и правата си; преместване мести и тях. Жива
      проверка в `secp/tools/ws_files_smoke.sh`.
- [x] Изчисляване на ефективни права по пътя (2026-09-24): `acl_effective`
      връща нивото на най-дълбокия приложим ред върху базата от ролята;
      `tree_acl_subject_rows` тегли редовете на субекта веднъж за
      пространството (използва се и от търсенето). Кеш/инвалидиране е
      следваща стъпка при нужда — виж G37.
- [x] UI за ACL в Next (2026-09-24): диалог „Споделяне" (`ShareDialog.tsx`)
      от бутона в списъка/прегледа на файл или папка. Показва редовете с
      ИМЕ/ИМЕЙЛ на субекта (сървърът ги дава — `tree_acl_to_json_named`,
      иначе UI-ът би показвал само числено id), добавя/сменя ред (потребител
      по имейл или роля в пространството; ниво 0..3; „и подпапките"), трие
      ред. Бутонът се показва само при `canShare` (owner/admin на екипно
      пространство) — иначе сървърът връща 403. `Dialog` получи `wide`
      вариант за таблицата. Ключовете са в 4-те езика.
- [x] Списъкът уважава ACL (2026-09-24): `act_fs_list` филтрира ДЕЦАТА по
      ефективно ниво — иначе коренът издава имената на заключени папки и
      UI-ът показва папка, която дава 403 при отваряне (същият дефект като
      при търсенето, хванат при живата проверка). Проверява се в
      `ws_files_smoke.sh`.
- [x] Fix: идемпотентно лично пространство (2026-09-24): `ws_personal_ensure`
      вдигаше 500 при паралелни `GET /v1/workspaces` (две заявки виждат „още
      няма личен" и втората удря `UNIQUE(slug)`). Сега при провален insert
      редът се прочита отново (insert-or-get), а съществуващо членство се
...
- [x] Модул `share`: линкове (uuidbaga токен, expiry, парола-чрез-hash, download count) (2026-09-24)
  - `tree/share_roles.baga` — чист модул: нива (1=четене, 2=качване), поддърво
    (`share_path_inside`), TTL, таван на свалянията, PBKDF2 парола, токен (32 байта →
    64 hex знака). Покрит от `tests/share_roles_test.baga` (20 проверки, в CI).
  - `share_links` — пази се **само хешът** на токена (като `idm_resets`): изтекъл dump
    не дава достъп. Времената са epoch секунди (BIGINT) — PG и boilaDB еднакво (G23).
  - API за собственика (ниво „споделяне" = 3 върху пътя, не роля): `GET/POST/DELETE
    /v1/fs/share`. Отговорът при създаване носи суровия токен/линк — единственото
    място, където се виждат.
  - Публично API без Bearer (`public=1`): `GET /s/{token}` (мета, не иска парола),
    `POST /s/{token}/unlock` (парола → краткотраен JWT `share`), `GET /s/{token}/view`,
    `GET /s/{token}/download` (брои сваляне), `PUT /s/{token}/upload` (само ниво 2).
  - „няма такъв", „изтекъл" и „отнет" дават един и същ 404 — по кода не се проверява
    кой токен съществува. Паролата не влиза в URL-а: носи се `pass_token` (claim
    `share` = id на линка), за да не я виждат рутери/логове.
  - Хигиена: изтрит възел/пространство отнема линковете си; преместване на поддърво
    мести пътищата им (`share_revoke_subtree`/`share_move_subtree`).
  - UI: раздел „Линкове" в диалога „Споделяне" (създаване, показване веднъж, копиране,
    отнемане) + публична страница `/share/<token>` (4 езика) — парола, списък, преглед,
...
- [x] „Cells" = екипно пространство с членове и роли (2026-09-25): това е
      workspace от Фаза 2 (`idm_workspaces`, роли owner/editor/viewer, ACL).
      Отделна таблица „cell" би повторила същото. Стаята на клетката е чатът
      на това пространство.
- [x] wsbaga gateway: канали per workspace; събития node.created/updated/deleted (2026-09-24)
- [x] chatbaga: стая per пространство (2026-09-25). Правилата (празен текст,
      таван 2000 байта, ключ `ws:<id>`) са в `chatbaga/chat_room.baga` —
      чист модул, `tests/chat_room_test.baga`. `chat_serve` НЕ се вдига:
      няма вход, а картите му не се споделят с HTTP нишката (G52).
      Историята е `chat_messages`. `GET/POST /v1/chat` иска ниво четене
      (и зрителят пише). Новата реплика стига по същия WS канал като
      `chat.message` — HTTP-ът само записва реда. UI: панел „Чат" до
      активността, 4 езика. Триене на пространство маха и репликите.
